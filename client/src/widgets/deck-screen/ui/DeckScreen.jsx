...
const CardDesk = () => {
    ...
    useEffect(() => {
        ...
        let active = true;
        api.deck.get()
            .then((deck) => { if (active) setTroops(deck); })
            .catch((error) => console.error("Failed to load deck", error));
        return () => { active = false; };
    }, [navigate, api, isAuthenticated]);
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-6">
                    ...
                    <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 p-2 sm:p-3 rounded-xl border border-indigo-200">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-indigo-700 text-sm sm:text-base font-semibold">{selectedTroop.attackSpeed}</span>
                        </div>
                    </div>
