...
import { useSimpleGameController } from "@/features/game";
...
export default function SimpleGameScreen() {
    ...
    const eventHandlersRef = useRef({ attack: () => {}, heal: () => {}, turn: () => {}, gameOver: () => {}, snapshot: () => {} });
    ...
    useSimpleGameController({ api, isAuthenticated, navigate, handlersRef: eventHandlersRef, hasLeftGameRef, damageTimeoutRef, healTimeoutRef, notify: showNotification });
    ...
    const extractMaxHP = (towers) => ({
        king: towers.king.maxHP,
        guard1: towers.guard1.maxHP,
        guard2: towers.guard2.maxHP,
    });
    ...
    const skipTurn = () => {
        ...
        api.game.skipTurn(localStorage.getItem("room_id")).catch((error) => showNotification(error.message));
    };
    ...
    const selectTroop = (troopName) => {
        ...
        if (troop.type === "healer") {
            api.game.heal({ roomId: localStorage.getItem("room_id"), troop: troop.name })
    ...
    const selectTarget = (target) => {
        ...
        api.game.attack({ roomId: localStorage.getItem("room_id"), troop: selectedTroop.name, target })
            .catch((error) => showNotification(error.message));
    ...
    const handleHeal = (msg) => {
        const { turn, player, opponent, healedTower, healAmount } = msg;
    ...
    const handleSkipTurn = (msg) => {
        const { turn, player1, player2 } = msg;
    ...
    const Tower = ({ id, type, health, maxHealth, isOpponent, onClick = undefined, disabled, className = "" }) => {
        ...
        return (
            <div
                className={`relative tower ${type} ${health <= 0 ? "grayscale" : ""} w-full h-full flex flex-col items-center justify-end ${!disabled ? "cursor-pointer" : ""} ${className}`}
