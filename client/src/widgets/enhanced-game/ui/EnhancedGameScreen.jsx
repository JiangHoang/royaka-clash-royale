...
import { useWebSocketContext } from "@/app/providers/websocket";
import { useEnhancedGameController } from "@/features/game";
import { LoadingScreen } from "@/shared/ui";

export default function EnhancedGameScreen() {
    const url = import.meta.env.BASE_URL;
    const navigate = useNavigate();
    const { api, isAuthenticated } = useWebSocketContext();

    const containerRef = useRef(null);
    const isPlayer1Ref = useRef(false);
    const hasShownTimeAnimRef = useRef(false);
    const hasLeftGameRef = useRef(false);
    const eventHandlersRef = useRef({ troop: () => {}, mana: () => {}, state: () => {}, gameOver: () => {}, snapshot: () => {} });

    const [user, setUser] = useState(/** @type {import('@/entities/player').Player | null} */ (null));
    const [opponent, setOpponent] = useState(/** @type {import('@/entities/player').Player | null} */ (null));
    ...
    useLayoutEffect(() => {
        ...
        if (!isGameInitialized || !container) return;

        const measureTiles = () => setTileSize(container.getBoundingClientRect().width / 22);
        measureTiles();
    ...
    useEnhancedGameController({ api, isAuthenticated, navigate, handlersRef: eventHandlersRef, hasLeftGameRef, notify: showNotification });
    ...
    const spawnTroop = (row, col) => {
        ...
        api.game.selectTroop({ roomId: localStorage.getItem("room_id"), troop: game.selectedTroop.name, x: col, y: row })
    ...
    // === Handle Game Over ===
    const handleGameOver = (result, response) => {
        setTimeout(() => {
            setGame((prev) => ({ ...prev, gameOver: true, winner: result.winner?.user.username ?? "", message: response.message }));
        }, 1000)
    };
    ...
    const handlePlayAgain = async () => {
        ...
        try { await api.game.playAgain(roomId); } catch (error) { showNotification(error.message); return; }
    ...
    eventHandlersRef.current = {
        snapshot: handleSetGameState,
        troop: handleSetTroop,
        mana: handleSetMana,
        state: handleSetMap,
    ...
    /** @param {Date | null} lastAttack */
    const attackedRecently = (lastAttack) => Boolean(
        lastAttack && Date.now() - lastAttack.getTime() >= 0 && Date.now() - lastAttack.getTime() < 450
    );


    // Tower component for reusability 
    const Tower = ({ type, health, maxHealth, isOpponent, isAttacking, attackRange, className = "" }) => {
        ...
        return (
            <div
                className={`relative tower ${type} ${health <= 0 ? "grayscale" : ""} w-full h-full flex flex-col items-center justify-end ${className}`}
            ...
            >
                <div className="w-full h-full flex flex-col items-center justify-center relative">
                    {isAttacking && (
                        <>
                            <div
                                className={`absolute left-1/2 top-1/2 rounded-full border-2 ${isOpponent ? "border-red-400 bg-red-400/10" : "border-cyan-300 bg-cyan-300/10"} animate-attack-range pointer-events-none`}
                                style={{ width: attackRange * 2, height: attackRange * 2 }}
                            />
                            <div className={`absolute left-1/2 top-1/2 h-2/3 aspect-square rounded-full ${isOpponent ? "bg-red-300/50" : "bg-cyan-200/50"} animate-tower-attack pointer-events-none`} />
                        </>
                    )}
    ...
    if (!isGameInitialized || !user || !opponent) return <LoadingScreen message="Loading battle..." />;
            ...
            >
                ...
                <div ref={containerRef} className="battle-container rounded-lg shadow-inner border-4 border-stone-600 overflow-hidden relative w-full aspect-square">
                    ...
                    <div className="grid-battlefield grid [grid-template-columns:repeat(22,1fr)] [grid-template-rows:repeat(22,1fr)] relative w-full aspect-square pointer-events-none">
                        {game?.map?.filter(e => e.typeEntity === "tower").map((tower) => {
                            const isEnemyTower = tower.owner !== localStorage.getItem("username");

                            const colStart = isPlayer1 ? 21 - tower.area.bottomRight.x + 1 : tower.area.topLeft.x + 1;
                            const rowStart = isPlayer1 ? 21 - tower.area.bottomRight.y + 1 : tower.area.topLeft.y + 1;
                            const colEnd = isPlayer1 ? 21 - tower.area.topLeft.x + 1 : tower.area.bottomRight.x + 1;
                            const rowEnd = isPlayer1 ? 21 - tower.area.topLeft.y + 1 : tower.area.bottomRight.y + 1;
                            ...
                            return (
                                ...
                                >
                                    <Tower
                                        ...
                                        maxHealth={tower.template.maxHP}
                                        isOpponent={isEnemyTower}
                                        isAttacking={attackedRecently(tower.lastAttack)}
                                        attackRange={tower.template.range * tileSize}
                        ...
                        {game?.map?.filter(e => e.typeEntity === "troop").map((troop) => {
                            ...
                            const isDisplayHP = troop.template.hp < troop.template.maxHP;
                            const health = troop.template.hp;
                            const maxHealth = troop.template.maxHP;
                            const isLowHP = health > 0 && health / maxHealth <= 0.2;
                            const isDead = health <= 0;
                            const isAttacking = attackedRecently(troop.lastAttack);
                            const attackRange = troop.template.range * tileSize;
                            ...
                            return (
                                ...
                                >
                                    {isAttacking && (
                                        <div
                                            className={`absolute left-6 top-6 rounded-full border-2 ${isEnemyTroop ? "border-red-400 bg-red-400/10" : "border-cyan-300 bg-cyan-300/10"} animate-attack-range pointer-events-none`}
                                            style={{ width: attackRange * 2, height: attackRange * 2 }}
                                        />
                                    )}
                                    {isDisplayHP && (
                                    ...
                                    <img
                                        ...
                                        className={`relative z-10 w-12 h-12 object-contain drop-shadow-lg ${isDead ? "grayscale opacity-50" : ""} ${isAttacking ? "animate-unit-attack" : ""}`}
                                    />
                                </div>
                                            ...
                                            <div className="p-2">
                                                <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                                                    <div className="flex items-center">
                                                        ...
                                                        <span className="text-[11px]">HP: {troop.maxHP}</span>
                                                    </div>
                                                    <div className="flex items-center">
