...
import { clearStoredSession, hasStoredSession } from "@/shared/lib";
...
export default function LobbyScreen() {
    ...
    const [user, setUser] = useState(/** @type {import('@/entities/user').User | null} */ (null));
    const [expMax, setExpMax] = useState(100);
    const [selectedMode, setSelectedMode] = useState(/** @type {import('@/features/matchmaking/api/contracts.js').GameMode} */ ("simple"));
    ...
    useEffect(() => {
        ...
        const offFound = api.match.onFound((match) => {
            localStorage.setItem("room_id", match.roomId);
            showNotification("Match found! Starting game...");
            setTimeout(() => navigate(selectedMode === "simple" ? "/game-simple" : "/game-enhanced"), 1000);
        });
        const offTimeout = api.match.onTimeout(() => {
            showNotification("Match timed out. Waiting for new match...");
            setIsJoiningGame(false);
        });
        if (isAuthenticated) {
            api.user.getCurrent().then(({ user: currentUser, maxExp }) => {
                setUser(currentUser);
                setExpMax(maxExp);
                localStorage.setItem("username", currentUser.username);
            }).catch((error) => showNotification(error.message));
        }
        return () => { offFound(); offTimeout(); };
    }, [api, navigate, selectedMode, isAuthenticated]);
    ...
    async function findMatch() {
        ...
        try {
            await api.match.find(selectedMode);
    ...
    async function handleLogout() {
        try { await api.auth.logout(); } catch { /* local logout still proceeds */ }
        clearStoredSession();
        navigate("/auth");
    }
    ...
    if (!user) return <LoadingScreen message="Loading player profile..." />;
